        ...
        {persons.map((person) => {
          ...
          return (
            <button key={person} type="button" onClick={() => onSelect(person)} className="rounded-xl border border-line bg-plane p-4 text-left transition-colors hover:border-brand-200 hover:bg-brand-50/50">
              ...
              <p className="mt-3 text-lg font-bold text-ink">月額 {formatYen(Math.round(monthly))}</p>
              {dollarMonthly > 0 && <p className="mt-0.5 text-[11px] font-semibold text-brand-700">うちドル建て {formatUsd(dollarMonthly)}/月</p>}
