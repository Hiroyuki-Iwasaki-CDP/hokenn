...
import { PREMIUM_FREQUENCY_LABEL, CONTRACT_TYPE_LABEL, RELATION_LABEL } from '../lib/status'
...
export default function PolicyDetail() {
  ...
  const { getPolicy, family, deletePolicy } = useInsurance()
  ...
  const insured = family.find((m) => m.id === policy.insuredMemberId)
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-ink-secondary">{policy.insurerName}</p>
            </div>
          </div>
      ...
      <DetailSection title="契約者と被保険者" icon={<Users size={17} />}>
        <FieldGrid>
          ...
          <Field
            label="被保険者"
            value={insured ? `${insured.name}(${RELATION_LABEL[insured.relation]})` : '—'}
          />
        </FieldGrid>
      </DetailSection>
      ...
      <DetailSection title="契約期間" icon={<CalendarRange size={17} />}>
        <FieldGrid>
          <Field label="契約開始日" value={formatDate(policy.startDate)} />
          <Field label="契約タイプ" value={CONTRACT_TYPE_LABEL[policy.contractType]} />
          <Field
            label="保障終了"
            value={
